...
export default function RankingEditor({
  ...
  const [title, setTitle] = useState(
    initialTitle || "私的イコラブ楽曲ランキング",
  );

  // 初期ランキングIDがある場合、それを復元する
  const [ranking, setRanking] = useState<RankingItem[]>(() => {
    if (initialRankingIds && initialRankingIds.length > 0) {
      return INITIAL_RANKING.map((item, index) => {
        const songId = initialRankingIds[index];
        if (songId) {
          const foundSong = songs.find((s) => s.id === songId);
          return { ...item, song: foundSong || null };
        }
        return item;
      });
    }
  ...
  useEffect(() => {
    if (!isMounted) return;
    // URLパラメータから初期化された場合はローカルストレージを無視する、あるいは
    // ユーザーが「自分のランキングを作る」モードに入った時にローカルストレージを使うなど
    // 今回はシンプルに、初期パラメータがない場合のみローカルストレージを読む（またはマージする）戦略にします
    if (initialRankingIds && initialRankingIds.length > 0) return;
  ...
  }, [isMounted, initialRankingIds]);

  useEffect(() => {
    if (!isMounted) return;
    // URLから来た場合でも、変更があれば保存する
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <span style={{ color: "#ff69b4", fontSize: "3rem" }}>♡</span>
              <div
                style={{
                  fontSize: "3rem",
                  fontWeight: "bold",
          ...
          >
            <span style={{ color: "#d8a0b0", fontSize: "1.2rem" }}>
              ♡ {currentDate || "---"} ♡
            </span>
            <div
              style={{
